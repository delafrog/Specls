...
#define LinearApproxOnSelectedData			0
#define GaussApproxOnSelectedData			1
#define LorentsApproxOnSelectedData			2
...
//	LorentsApproximator *p_loraprx;
	...
	double selX1, selX2; // значения выбранного X и интервала DX;
	...
	void SetUpParWinwow(); // настройка окна параметров графика
	...
	vecT< vecdb > & X() { return Xdata; }
	vecT< vecdb > & Y() { return Ydata; }

	void setSelActText(const QString &text);

public	slots:
	void setZoomMode() { cntxMenuItAc = ZoomGraphContextMenuAction;  ZoomFrame = true; update(); }
	void setDfltMode() { cntxMenuItAc = NoGraphContextMenuAction; ZoomFrame = false; update(); }
	...
	void getdata2clopboard();
	
	void addPeakData(); // добавить данные для выбранного пика  

	void ParWindowShow(); // вывеси окно с параметрами графика

	void setUpApprxTblCnt(int tp_in); // установить тип аппроксимации
	void setUpLinTbl();
	void setUpGssTbl();
	void setUpLorTbl();

	void setActiveChanel(int id_chn);
...
private:
	...
	QWidget *ParWindow;// окно с параметрами графика
	//QComboBox *CBoxApprxTypeSelect; // выбор типа аппроксимации

	Ui::PltPrmWnd *ui_ParWindow;

	QImage *ActivePlot_bar; // прямоугольник с цветов активного графика
	...
	int chanel_id; // номер графика в обработке аппроксимации
	int aprxtype; // идентификатор типа аппроксимации
	...
	QAction *graph2clipboard; // сохранить в буфер

	QAction *ShowParamWindow; // показать окно с параметрами графика : размеры выделенной области и данными аппроскимации
