...
//____________________________________
template <class T>
vecT<T>  SparseMatr<T>::CGMNORM2_solve(vecT<T> &b,T rmin )	// решение системы линейных уравнений Ax=b, где A- текущая разреженная матрица (сопряжённые градиенты к нормализованным уравнения)
{
	SparseMatr<T> B;
	vecT<T> z,pk,yk,rk,xk,bk,xh;
	T mk,nk,hk,res;
